...
Vue.component('LinkExistingAccount', {
    template: `
        <form @submit.prevent="$emit('submit')">
            <h5 class="subtitle is-5">Link your Arc Account</h5>
            <div class="field">
            ...
            <div class="field">
                ...
                <div class="control">
                    <input
                        :value="propertyId"
                        @input="$emit('update:propertyId', $event.target.value)"
                        class="input"
                        minlength="8"
                        required>
                </div>
            ...
            <p>
                Your widget ID can be found
                <a :href="widgetPageUrl" target="_blank">here</a>.
            </p>
            <p>
                For example, the widget ID for the below script tag would be
                <code>GjcF5zo4</code>
            </p>
            <p><code v-text="widgetScript" /></p>
            <button
                class="button"
                type="submit">
                Submit
...
new Vue({
    el: '#arc-wp-admin-app',
    template: `
        <div id="arc-wp-admin-app" class="container">
            ...
            <RegistrationSuccess
                ...
                :portalUrl="portalUrl" />
            <LinkExistingAccount
                v-else-if="state === STATES.LINK_EXISTING_ACCOUNT"
                @submit="linkArcAccount"
                :email.sync="arcEmail"
                :propertyId.sync="propertyId"
                :portalUrl="portalUrl" />
            <form @submit.prevent="registerArcAccount" v-else>
                ...
                <p>
                    ...
                    <a @click="goToLinkArcAccountPage">Already have an Arc account?</a>
                </p>
                <p>
    ...
    methods: {
        ...
        goToLinkArcAccountPage () {
            // Clear the autogenerated propertyId, since user will enter their own
            this.propertyId = ''
            this.transition(STATES.LINK_EXISTING_ACCOUNT)
        },
